...
async def handle_chat(request: ChatRequest, db = Depends(get_database)):
    ...
    try:
        ...
        if is_blueprint_request:
            ...
            execution_data = result.get("execution_results", {})
            
            # Map the execution insights directly onto the blueprint response mapping
            blueprint_data["execution"] = execution_data
